...
inputx = np.reshape(am_train[0:90,0:6], [-1,6,1])
inputy = np.reshape(am_train[0:90,6:12], [-1,6,1])

testx = np.reshape(am_train[90:,0:6], [-1,6,1])
testy = np.reshape(am_train[90:,6:12], [-1,6,1])
...
with tf.Session() as sess:
    ...
    for i in range(1000):
        ...
        print 'accu:',sess.run(accuracy, feed_dict={x:testx, y:testy})
